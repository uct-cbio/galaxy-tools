...
def add_file( dataset, json_file, output_path ):
 ...
 #                  return
                if ( test_ext in unsniffable_binary_formats ) and not (dataset.file_type == 'binseq.zip' or dataset.file_type == 'zip'):
                    file_err( "Invalid 'File Format' for archive consisting of binary files - use 'Binseq.zip' or 'zip'", dataset, json_file )
                    return
                ...
                if not ( dataset.file_type == 'binseq.zip' or dataset.file_type == 'txtseq.zip' or  dataset.file_type == 'zip'):
                    file_err( "You must manually set the 'File Format' to either 'Binseq.zip' or 'Txtseq.zip' or 'zip' when uploading zip files", dataset, json_file )
                    return
                data_type = 'zip'
